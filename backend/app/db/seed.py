"""Project data + seeding logic, shared by scripts/seed_projects.py and
the app's own startup routine (app/main.py).

Kept idempotent on purpose: re-running it never creates duplicates (it
skips any slug that already exists), so it's safe to call automatically
every time the app boots — important on hosts like Render's free tier,
where the local SQLite file does not survive a redeploy or restart.
"""
